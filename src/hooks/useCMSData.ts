...
// Hook générique pour gérer les données du CMS
export function useCMSData<T>(key: string, defaultValue: T) {
  ...
  useEffect(() => {
    try {
      const savedData = cmsStorage.getData(key, defaultValue);
      setData(savedData);
    } catch (err) {
  ...
  const saveData = (newData: T) => {
    try {
      cmsStorage.saveData(key, newData);
      setData(newData);
      setError(null);
    } catch (err) {
...
// Constantes par défaut pour éviter les re-créations
...
// Hooks spécialisés utilisant des constantes
export function useHomepageData() {
  return useCMSData('homepage-sections', DEFAULT_HOMEPAGE_SECTIONS);
}

export function useProjectsData() {
  return useCMSData('projects', DEFAULT_PROJECTS);
}

export function useAboutData() {
  return useCMSData('about', DEFAULT_ABOUT_DATA);
}

export function useServicesData() {
  return useCMSData('services', DEFAULT_SERVICES);
}

export function useContactData() {
  return useCMSData('contact', DEFAULT_CONTACT_DATA);
}